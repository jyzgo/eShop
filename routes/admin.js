      ...
      if (product) {
        ...
        if (cartIndex > -1) {
          req.session.cart[cartIndex].quantity =
            parseInt(req.session.cart[cartIndex].quantity) + productQuantity;
          req.session.cart[cartIndex].totalItemPrice =
            productPrice * parseInt(req.session.cart[cartIndex].quantity);
        } else {
          // Doesnt exist so we add to the cart session
          req.session.cartTotalItems =
            req.session.cartTotalItems + productQuantity;
  ...
  async.eachSeries(
    ...
    (cartItem, callback) => {
      ...
      if (cartItem.itemQuantity === 0) {
        ...
        db.products.findOne(
          { _id: common.getId(cartItem.productId) },
          (err, product) => {
            if (err) {
            ...
            if (product) {
              ...
              if (req.session.cart[cartItem.cartIndex]) {
                ...
                req.session.cart[cartItem.cartIndex].totalItemPrice =
  ...
  db.products.findOne(
    ...
    (err, product) => {
      ...
      db.products.count(
        {
          productPermalink: req.body.frmProductPermalink,
          _id: { $ne: common.getId(product._id) }
        },
        (err, count) => {
          if (err) {
            console.info(err.stack);
              ...
              db.products.update(
                ...
                (err, numReplaced) => {
                  if (err) {
                    ...
                    req.session.messageType = "danger";
                    res.redirect(
                      "/admin/product/edit/" + req.body.frmProductId
                    );
                  } else {
                    // create lunr doc
                    let lunrDoc = {
...
router.post("/user/insert", common.restrict, (req, res) => {
  ...
  db.users.findOne({ userEmail: req.body.userEmail }, (err, user) => {
    if (user) {
      ...
      console.error(
        colors.red("Failed to insert user, possibly already exists: " + err)
      );
...
router.get("/settings", common.restrict, (req, res) => {
  res.render("settings", {
    ...
    config: common.getConfig(),
    footerHtml:
      typeof common.getConfig().footerHtml !== "undefined"
        ? escape.decode(common.getConfig().footerHtml)
        : null,
    googleAnalytics:
      typeof common.getConfig().googleAnalytics !== "undefined"
        ? escape.decode(common.getConfig().googleAnalytics)
        : null
  });
});
  ...
  db.products.findOne(
    ...
    (err, product) => {
      ...
      if (product.productOptions) {
        ...
        db.products.update(
          { _id: common.getImages(req.body.productId) },
          { $set: { productOptions: JSON.stringify(optJson) } },
          (err, numReplaced) => {
            if (err) {
              console.info(err.stack);
            }
...
router.post("/settings/pages/update", common.restrict, (req, res) => {
  ...
  if (req.body.page_id) {
    ...
    db.pages.findOne({ _id: common.getId(req.body.page_id) }, (err, page) => {
      ...
      if (page) {
        db.pages.update(
          { _id: common.getId(req.body.page_id) },
          { $set: doc },
          {},
          (err, numReplaced) => {
            if (err) {
...
router.post(
  ...
  (req, res, next) => {
    ...
    if (req.file) {
      ...
      db.products.findOne(
        { _id: common.getId(req.body.directory) },
        (err, product) => {
          if (err) {
            console.info(err.stack);
          }
